...
export type Product = { name: string; note: string; price: string };
                ...
                <Reveal key={p.name} delay={i * 80}>
                  <article className="group flex h-full flex-col">
                    <div className="relative aspect-square overflow-hidden rounded-sm bg-muted">
                      <div
                        className="absolute inset-0 bg-cover bg-center opacity-90 transition-all duration-700 group-hover:scale-105 group-hover:opacity-100"
                        style={{ backgroundImage: `url(${image})`, filter: `hue-rotate(${i * 18}deg) saturate(${0.85 + i * 0.05})` }}
                      />
                      <button
