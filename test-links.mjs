...
(async () => {
  ...
  const hrefs = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('a')).map(a => a.getAttribute('href')).filter(href => href && href.startsWith('/'));
  });
  
  // Remove duplicates and filter for product and category links
  const uniqueLinks = [...new Set(hrefs)].filter(href => href.includes('/product/') || href.includes('/category/'));
